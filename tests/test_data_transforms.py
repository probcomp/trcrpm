...
def test_activate_glm():
    ...
    rng = np.random.RandomState(2)
    trcrpm = TRCRP_Mixture(chains=2, lag=3, variables=FRAME.columns, rng=rng,
        dependencies=[['a','b','c']])
    trcrpm.incorporate(FRAME)

    # Variable should regress on itself by default.
    trcrpm.activate_glm('a')
    index_a = trcrpm._variable_to_index('a')
    for state in trcrpm.engine.states:
        dim = state.dim_for(index_a)
        assert dim.cctype == 'linear_regression'
        assert set(dim.inputs[1:]) == set([
            trcrpm._variable_to_index('a', 1),
            trcrpm._variable_to_index('a', 2),
            trcrpm._variable_to_index('a', 3),
        ])

    # Variable can regress on other variables.
    trcrpm.activate_glm('b', covariates=['a','c'])
    index_b = trcrpm._variable_to_index('b')
    for state in trcrpm.engine.states:
        dim = state.dim_for(index_b)
        assert dim.cctype == 'linear_regression'
        assert set(dim.inputs[1:]) == set([
            trcrpm._variable_to_index('a', 1),
            trcrpm._variable_to_index('a', 2),
            trcrpm._variable_to_index('a', 3),
            trcrpm._variable_to_index('c', 1),
            trcrpm._variable_to_index('c', 2),
            trcrpm._variable_to_index('c', 3),
        ])

    trcrpm.simulate_ancestral([0,1,2], ['a','b'], 10, multiprocess=0)
    trcrpm.simulate([0,1,2], ['a','b'], 10, multiprocess=0)
